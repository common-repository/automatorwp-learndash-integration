...
class AutomatorWP_LearnDash_User_Group_Leader extends AutomatorWP_Integration_Action {
    ...
    public function register() {
        ...
        automatorwp_register_action( $this->action, array(
            ...
            'label'             => __( 'Make user the leader of a group', 'automatorwp-learndash' ),
            'select_option'     => __( 'Make user the <strong>leader</strong> of a group', 'automatorwp-learndash' ),
            /* translators: %1$s: Group. */
            'edit_label'        => sprintf( __( 'Make user the leader of a %1$s', 'automatorwp-learndash' ), '{group}' ),
            /* translators: %1$s: Group. */
            'log_label'         => sprintf( __( 'Make user the leader of a %1$s', 'automatorwp-learndash' ), '{group}' ),
            'options'           => array(
                'group' => array(
                    'default' => __( 'group', 'automatorwp-learndash' ),
                    'fields' => array(
                        'post' => automatorwp_utilities_post_field( array(
                            'name'              => __( 'Group:', 'automatorwp-learndash' ),
                            'option_none'       => false,
                            'option_custom'         => true,
                            'option_custom_desc'    => __( 'Group ID', 'automatorwp-learndash' ),
                            'post_type'         => 'groups',
                            'placeholder'       => __( 'Select a group', 'automatorwp-learndash' ),
                            'default'           => ''
                        ) ),
                        ...
                        'leader_role_assignment' => array(
                            'name' => __( 'If user doesn\'t have the "Group Leader" role:', 'automatorwp-learndash' ),
                            'type' => 'select',
                            'options' => array(
                                'nothing'   => __( 'Do nothing', 'automatorwp-learndash' ),
                                'add'       => __( 'Add the role to their existing roles', 'automatorwp-learndash' ),
                                'set'       => __( 'Replace their existing roles with the "Group Leader" role', 'automatorwp-learndash' ),
                            ),
                            'default' => 'nothing'
                        ),
                    )
