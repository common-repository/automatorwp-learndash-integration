...
class AutomatorWP_LearnDash_Complete_Quiz extends AutomatorWP_Integration_Trigger {
    ...
    public function register() {
        ...
        automatorwp_register_trigger( $this->trigger, array(
            ...
            'label'             => __( 'User attempts, passes or fails a quiz', 'automatorwp-learndash' ),
            'select_option'     => __( 'User <strong>attempts, passes or fails</strong> a quiz', 'automatorwp-learndash' ),
            /* translators: %1$s: Operation (attempts, passes or fails). %2$s: Post title. %3$s: Number of times. */
            'edit_label'        => sprintf( __( 'User %1$s %2$s %3$s time(s)', 'automatorwp-learndash' ), '{operation}', '{post}', '{times}' ),
            /* translators: %1$s: Operation (attempts, passes or fails). %2$s: Post title. */
            'log_label'         => sprintf( __( 'User %1$s %2$s', 'automatorwp-learndash' ), '{operation}', '{post}' ),
            ...
            'options'           => array(
                'operation' => array(
                    ...
                    'fields' => array(
                        'operation' => array(
                            'name' => __( 'Operation:', 'automatorwp-learndash' ),
                            'type' => 'select',
                            'options' => array(
                                'attempt'     => __( 'attempts', 'automatorwp-learndash' ),
                                'pass'    => __( 'passes', 'automatorwp-learndash' ),
                                'fail'    => __( 'fails', 'automatorwp-learndash' ),
                            ),
                            'default' => 'attempt'
                        ),
                    )
